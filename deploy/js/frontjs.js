...
/**
Version of the FrontJS.
//*/
var FRONT_VERSION = "0.9.0";
...
var Activity = 
{
	...
	Stops all loops.
	//*/
	stop: function() { var a = Activity; a.cancelItv(); if (a.hasReqAnimFrame) a.cancelRAF(); },
	
	/**
	Clears the execution list.
	//*/
	clear: function() { var a = Activity; a.m_list = []; },
	...
	run:function(p_callback,p_duration,p_delay,p_run_on_background)
	{
		...
		{	
			...
			if(n.elapsed >= n.duration) { p_callback(n); Activity.remove(n); return;	}
		};
		Activity.add(n,p_run_on_background);
		return n;
	},
	...
	//*/
	delay: function(p_callback,p_delay,p_run_on_background,p_args)
	{		
		var al = p_args==null ? [] : p_args;
		//for(var i=3;i<arguments.length;i++) al.push(arguments[i]);		
		return Activity.run(function(n) 
		{			
	...
	iterate: function(p_callback,p_list,p_step,p_timeout,p_run_on_background)
	{
		...
		return Activity.run(function(n) 
		{
			for(var i=0;i<stp;i++)
			{
				if(k>=p_list.length) { Activity.remove(n); break; }
	...
	//*/
	cancelRAF: function() { var a = Activity; if (a.RAFId >= 0) window.cancelAnimationFrame(a.RAFId); a.RAFId = -1; },
	
	/**
	Cancels the setIntervalLoop.
	//*/
	cancelItv: function() { var a = Activity; if (a.itvId >= 0) window.clearInterval(a.itvId); a.itvId = -1; },
};
...
{	
	...
	Initialize all data and callbacks.
	//*/
	init: function(){window.Animation = Animation; },
	...
	{  
		...
		var res = a.run(function(n)
		{
			if(!has_start)
			{
				Animation.clear(p_target,p_property,[n]);				
...
var Front =
{
	...
	controller:
	{
		...
		notify: function(p_path,p_event,p_target,p_data)
		{
			...
			for(var i=0;i< ref.list.length;i++)
			{
				...
				if(c.on != null)
				{
					//Event do not belongs to this controller permitted list.
	...
	view: 
	{
		...
		/**
		Searches the target using the given query. If target is null, Front.root is used instead.
		//*/
		query: function(p_query,p_target) 
		{ 
			var t = p_target==null ? Front.root : p_target; 
			if(t==null) return []; 
			return t.querySelectorAll(p_query); 
		},
	...
	setRoot: function(p_root)
	{
		if(ref.root != null)
		{
			...
			if(ref.observer!=null)ref.observer.disconnect();
		}		
		...
		if(ref.root != null)
		{
			...
			if(ref.observer!=null)ref.observer.observe(ref.root,config);			
	...
	//*/
	initialize: function(p_root,p_events,p_allow_mutation)
	{		
		ref = this;
		...
		var has_mutation = p_allow_mutation==null ? false : p_allow_mutation;
	...
	/**
	Searches the document using the given query.
	//*/
	query: function(p_query) { return document.querySelectorAll(p_query); },
	...
	/**
	Shortcut to the Activity class.
	//*/
	activity: Activity,
	
	/**
	Shortcut to the Animation class.
	//*/
	animation: Animation,
